...
def page4():
    ...
    with open("../input/rev_mapping_manual_mit_to_controls_final.json", 'r') as f:
    ...
    df.to_csv("../output/iag_controls_rec.csv", index=False)
    csv = df.to_csv(index=False).encode('utf-8')

    
    df2 = pd.read_csv("../output/iag_controls_rec.csv")
    st.dataframe(df2)
    st.download_button(
        label="Download CSV",
        data=csv,
